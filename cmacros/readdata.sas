 ...
 * oriname: specify the file name (with extension name) in which the data would be input.
...
%macro resolvefile() /minoperator;
   ...
   %else %let fullname=&oriname;
    /*parse the file name and extension name*/
   ...
   /*if the position >33, it suggest the file name is over 32 char*/
   %if &position > 33 %then
      %do;
   ...
   %else %if &type in (XLS XLSB XLSM XLSX) %then
      %do;
         ...
         proc sql noprint;
            ...
            from sashelp.vtable
            where libname= "TEMPLB" and memname not contains "_FilterDatabase";
            %let num=&sqlobs;
         quit;
