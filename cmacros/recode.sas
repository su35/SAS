...
%macro defi_code();
	%local dnum i ;
	%if %existsVar(dn=&dn, var=dataset)  %then  %do;
		proc sql noprint;
	...
	%do i=1 %to &dnum;
		proc sort data=&dn out=work.rc_tmp;
			by vid;
			%if &dnum>1 %then where dataset="&&rcdn&i"%str(;) ;
		run;
		%local tnum tnum2;
		proc sql noprint;
		...
		data work.rc&outfile;
			length dataset $32;
			set work.rc_tmp end=eof;
			where %if %existsVar(dn=&dn, var=dataset)  %then type="char" or; not missing(value_n);
			by vid;
			length code d r $&tnum ;
			retain newv d r;
			%if &dnum>1 %then dataset="&&rcdn&i"%str(;);
			if _N_=1 then do;
				d= "drop ";
				r="rename ";
			end;
			...
			if first.vid then do;
				...
				code="select ("||trim(variable)||"); when ('"||trim(value)||"') "||strip(mapvar)||"="||trim(left(value_n))||";";
				d=catx(" ", d, variable);
				r=catx(" ", r, trim(variable)||"_n="||trim(variable));
			end;
			else if last.vid=0 then do;
				if missing(value_n) then value_n=newv;
				code=" when ('"||trim(value)||"') "||strip(mapvar)||"="||trim(left(value_n))||";";
			end;
			else do; 
				if missing(value_n) then value_n=newv;
				code=" when ('"||trim(value)||"') "||strip(mapvar)||"="||trim(left(value_n))||";
						otherwise "||strip(mapvar)||"=. ; end;";
			end;
			output;
			newv+1;
			if eof then do;
			...
			keep dataset variable value value_n code;
		run;
		filename code %if &dnum>1 %then "&path.&&rcdn&i.._&outfile..txt"%str(;);
						%else "&path.&outfile..txt"%str(;);
		data _null_;
			set work.rc&outfile/* end=eof*/;
			rc=fdelete("code");
		...
		%if %sysfunc(exist(&outfile)) %then %do;
			proc sql noprint;
				%if &dnum>1 %then 
				delete from &outfile where dataset="&&rcdn&i";
				%else drop table &outfile;
				;
			quit;
		%end;
...
%macro radn_code();
	...
	data work.rc&outfile(rename=(table=variable));
		length dataset $32;
		...
		if first.table then do;
			n=1;
			code=trim(table)||"_n="||trim(left(n))||"*("||trim(table)||'="'||trim(value)||'")+';
			d=catx(" ", d, table);
			r=catx(" ", r, trim(table)||"_n="||trim(table));
		end;
		else if last.table then code=trim(left(n))||"*("||trim(table)||'="'||trim(value)||'");';
		else code=trim(left(n))||"*("||trim(table)||'="'||trim(value)||'")+';
		value_n=n;
		n+1;
		if eof then do;
			droplist=trim(d)||";";
