...
* Define_Hearder = define file header metadata
...
%macro make_define(path=, metadata=);
    %local standard;
    %if %superq(metadata) ne %then %do;
        %if %superq(path) ne %then %do;
            %if %sysfunc(fileexist(&path\&metadata)) %then
                libname templib xlsx "&path\&metadata"%str(;);
            %else %do;
                %if %sysfunc(fileexist(&pdir\&metadata)) %then
                    libname templib xlsx "&pdir.&metadata"%str(;);
                %else %do;
                    %put ERROR: ==The metadata file could be found==;
                    %return;
                %end;
            %end;
        %end;
    ...
    %if %symexist(xsldefine) = 0 %then %let xsldefine =define2-0-0.xsl;
    ...
    data work.md_defineHeader;
        ...
        if upcase(standard) = 'ADAM-IG' then
            ...
            put "ERR" "OR: CDISC standard undefined in Define_Hearder";
    run;
    ...
    data work.md_VariableLevel;
        ...
        else if "&standard" = "ADAM" and origin ne '' and comment = '' then 
            comment = "SOURCE: " || left(trim(origin)); 
        if "&standard" = "ADAM" and origin = '' and comment ne '' then 
    ...
    %if "&standard" = "ADAM" %then %do;
        ...
        data work.md_analysisresults;
            ...
            drop resultnum;
            retain resultnum;
            if first.displayid then resultnum = 0;
            resultnum + 1;
        ...
        **   2) To get the link information for the analysis reference   */   
        ...
        proc sort data = templib.ExternalLinks 
                 (drop = LeafRelPath SupplementalDoc AnnotatedCRF
                   rename=(title=doctitle leafid=displayid leafpageref=dsplylfpgref 
        ...
        proc sort  data = templib.ExternalLinks  
                (drop = LeafRelPath SupplementalDoc AnnotatedCRF
                rename=(title=reftitle leafid=refleafid leafpageref=reflfpgref 
...
/* Create computation method section*/
    ...
    data work.md_valuelevel;
        ...
        **   take priority.  Otherwise set the MethodOID to the comment oid;
        if computationmethodoid ne '' then methodoid = computationmethodoid;
    ...
    data work.md_whereclause3;
        ...
        *--- default softhard to Soft if not specified;
        if softhard eq '' then softhard = 'Soft';
    ...
    data work.md_itemdef;
        ...
        if upcase(origin)=:'CRF PAGE' then do;
            if "&standard" = "ADAM" then
        ...
        else if "&standard" = "ADAM" and upcase(origin) not 
                in:('DERIVED', 'ASSIGNED', 'PROTOCOL', 'EDT') then do;
    ...
    */    
        ...
        else if "&standard" = "ADAM" and upcase(origin) 
                not in:('DERIVED', 'ASSIGNED', 'PROTOCOL',  'EDT') then do;
