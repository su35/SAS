...
data random(index=(usubjid));
    set ori.random (keep=studyid usubjid treat);
    length rand_date 8;
...
proc sql;
    ...
    select e.studyid as studyid, e.usubjid as usubjid, put(e.subjid,8.-L) as subjid,
            ...
            .  as dthday, case when f.died = "1" or t.deathda ne '' then 'Y' else "" end as Dthfl,
            ...
            case upcase(r.treat) when "TOPIRAMATE" then "T" when "PLACEBO" then "P" 
                    else "" end as armcd, 
            r.treat as arm,   
        ...
        from (ori.enrl as e left join ori.tlfb as tl on e.usubjid=tl.usubjid) left join 
                ((random as r inner join ori.term as t on r.usubjid=t.usubjid) left join 
                   ori.fup as f on r.usubjid=f.usubjid)
...
/*run;*/
data _suppdm;
    call missing (rand_date);
    set ori.enrl(keep=studyid usubjid educatyr maritals employ30  eligible) ; 
    set  random(keep= usubjid  rand_date) key=usubjid;
    length  qorig  $ 8 qnam  $ 10 qval $ 30 qlabel $ 27;
    qnam = "EDUCATYR"; 
    qlabel = "Years of Formal Education"; 
    qorig ="CRF 01";
    if anyalpha(educatyr) then educatyr = "";
    if missing(educatyr) then call missing(qval); 
    else qval=educatyr;
    output;
    ...
    if qval="." then call missing(qval);
    output;
...
data _ae;
    ...
    merge ori.aelog (where=(aenum ne 99)  in=inae) 
    ...
    %let sevlist=%sysfunc(tranwrd(&sev, %str( ), %str(,)));
    %let serlist=%sysfunc(tranwrd(&ser, %str( ), %str(,)));
    aesev =left(max(&sevlist));
    aeser =left(max(&serlist));
...
data ae;
    ...
    if aestdtc="." then call missing(aestdtc);
    aeendtc = put (rand_date + aerdte, E8601DA10.-L);
    if aeendtc="." then call missing(aeendtc);
    aeterm = ptname;
    aedecod = aeevnt;
    aeptcd = ptcode;
    aesoc = socname;
...
data _ex ; 
    ...
    if first.usubjid then do;
        call missing(exdose,  exstdy,  _date);
    end;

    do i=1 to 7;
        if anyalpha(dos(1,i)) then dos(1,i) ="0" ;
        if anyalpha(dos(2,i))  then dos(2,i) ="0" ;
...
run;
proc sql noprint;
    select ifc(type="char", catx(" ",name, "$",length), catx(" ", name, length))
    into :lblen separated by " "
    from (select name, type, length
              from dictionary.columns
             where libname=upcase("&pname") and memname = "EMPTY_LB");
quit;
...
data _ur;
    ...
    do i=1 to 5;
        ...
        if not missing(ch(i)) then do;
            ...
            call missing(urorres, urorresu, urstresc, urstresn);
        end;
        output;
    end;
    do i=1 to 2;
        urtestcd = nvar(i); urtest = nvar(i);
        if not missing(nu(i)) and not(prxmatch('/[a-z]/i', nu(i))) then do;
             urorresu= "ng/ml"; 
            urorres = nu(i);  
            urstresc = "1"; 
            urstresn = nu(i);
    ...
    if not missing(ampconf) or not missing(amp) then do;
        urtestcd = "AMPHET"; URTEST =  "AMPHETAMINE"; 
        if not missing(ampconf) and not(prxmatch('/[a-z]/i', ampconf)) then do;
            urorres = ampconf; 
            urorresu= "ng/ml"; 
            urstresc = "1"; 
            ...
            urorres = amp;
            urstresc = amp;
            call missing(urorresu, urstresn);
        end;
        output;
    end;
...
run;
/**/
%let exclude=aehlgt aehlgtcd aehlt aehltcd dthdtc dthfl qval;
%CheckMissing()
%ReLenStd(SDTM)
...
data adsl;
    ...
    if randdtc ne "" then do;
        ...
        call missing(randdt,    trtsdt,  trtedt);  
        randfl = "N";  complfl ="N";
    end;
    trt01p = arm;
