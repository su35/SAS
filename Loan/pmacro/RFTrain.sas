...
** tn: the number of trees, the default is 50 
** tsize: the size of each tree(number of variables), the default is square root of the variables 
** obs: obs for each single tree, the default is 0.6 (60%)
** sub: the selected method, the default is hpsplit. if any value is assigned, then split.
** pth: the output path, the default is the outfile fold under project fold 
** *************************************************************************/
%macro RFTrain(tdn, pdn=vars, tn=50, tsize=, obs=0.6, vlist=, pth=, sub=, criterion=,
	...
	proc sql noprint;
		select 	variable, %if %superq(sub)= %then ifc(class="nominal" or class="binary", "nom", "int");
							%else class;
		into :target, :tlevel
		from &pdn
	...
	proc sort data=RFVarImportn;
		by variable;
	run;


	%if %superq(tsize)= %then %do;
		data _null_;
			set vars nobs=obs;
			where &where;
			call symputx("tsize", ceil(sqrt(obs)));
		run;
	%end;
	...
	%do i=1 %to &tn;
		...
		proc surveyselect data=&pdn(where=(&where)) 
				out=work.rft_tvar sampsize=&tsize noprint;
		quit;

		%if %superq(sub)= %then %do;
			data work.rft_tvar;
				set work.rft_tvar;
		...
		proc sort data=work.rft_importance%if %superq(sub)= 
