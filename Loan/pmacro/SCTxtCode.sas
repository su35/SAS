/* **********************************************************************************************
*  Macro SCTXTCode: Modified from Credit Risk Scorecards: Development and 
...
* **********************************************************************************************/
%macro SCTxtCode(scdn, basepoints, baseodds, pdo, filename, IntOpt);
	options nonotes;
	proc sort data=&scdn;
		by model_item bin;
	run;
	...
	data _null_;
		set &scdn nobs=nx;
		by model_item bin;
		where ori_var not is missing or upcase(model_item)="INTERCEPT";
		file "&pout.&filename.n.txt";
		length cond $300.;
		length value $300.;
		retain lastborder;
		if _N_ =1 then do;
		...
		if upcase(model_item)="INTERCEPT" then do;
			...
			/* *********  The rule   ***************/
			if b_var=ori_var then cond='IF '||trim(ori_var)||' = ('|| trim(left(bin))||') ';
			else if not missing(cluster) then cond = 'IF '||trim(ori_var)||' in ( '|| compbl(cluster) ||')' ;
			else do;
				if last.model_item then do;
					cond='IF '||trim(ori_var)||' GT ('|| compress(lastborder)||')';
					lastborder=.;
				end;
				else do; 
					cond='IF '||trim(ori_var)||' LE ('||compress(border) || ') ';
					lastborder=border;
				end;
			end;
			put "      " cond value;
	...
	run;
	 %if %existsVar(dn=&scdn, var=cvalue) %then %do;
		data _null_;
			set &scdn nobs=nx;
			by model_item bin;
			where ori_var not is missing or upcase(model_item)="INTERCEPT";
			file "&pout.&filename.c.txt";
			length cond $300.;
			length value $300.;
			...
			if upcase(model_item)="INTERCEPT" then do;
				...
				/* *********  The rule   ***************/
				if not missing(cvalue) then do;
					if not missing(cluster) then 
							cond = 'IF '||compress(ori_var)||' in ( '|| compbl(cvalue) ||')' ; 
					else cond='IF '||compress(ori_var)||' = ('|| compress(cvalue)||') '; 
				end;
				else if not missing(border) then  do;/* continuous */
	...
	%end;
	options notes;
	%put NOTE: == File &pout.&filename..txt was created ==;
